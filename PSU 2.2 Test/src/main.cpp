 ...
 *  Add short-circuit protection (if Rl < 1 ohm)
 *  Fixed code to eliminate precision adjustment - coded but not working
 *********************************************************************************/
// Definitions
//===============================================================================
  #include <iostream> 
...
//  int j = 0 ;                       // This is to slow down the screen polling
  ...
  float Vsense2 ;                    // For the final Voltage output display decimal
  float Vsense1 ;
  float Isense1 ;
  float Isense2 ; 
  int MAV2, MAV1 ;
...
//  gpio_num_t ADC_out_ref = (gpio_num_t) 25;
  ...
  const unsigned long Delay1 = 15 ;              // This is for the delay waiting for the quadrature pulse 10 ms is optimal
  const unsigned long DelayRST = 1;              // This is the length of the reset pulse to the 4013
  const unsigned long Delay2 = 5 ;              // This is for the delay waiting for the quadrature pulse 10 ms is optimal
...
void Check_current();
// void Draw_Button_Ch1();
// void Draw_Button_Ch2();
...
void Check_Rot_Enc() 
{
  digitalWrite(Reset1, LOW);      // Make sure CD4013 is enabled
...
void Draw_Button_Spare_pressed()
  {
    tft.fillRect( 220, 200, 82, 32, TFT_BLACK);  
    tft.fillRect(222, 203, 80, 30, TFT_BROWN1);    //base for Channel 1
    tft.fillRect(222, 206, 80, 23, TFT_GOLD1);     // Button for Channel 1
    tft.setFreeFont(FSS9);
    tft.setTextColor(TFT_BLACK, TFT_GOLD1);
    tft.drawString("50mV", 238, 208, 2);
    // Precise = 1 ;
  }
void Check_current()  
{
...
//  Isense1 = analogRead( Is1 );
  ...
  else if ( Iout >= Ilimit )
  {
    ...
    CL = 1 ;
    Vset = Ilimit * Vsense2 / Isense2 ; // calculated on the fly. Rl = Vsense2 / Isense2
...
void Display_Output() 
  ...
  {     
      ...
      tft.drawFloat( abs(Vsense2), 2, 175, 65, 4 );
  //    tft.drawString("V", 125, 175, 4);
   ...
   //   tft.drawString("V", 125, 45, 4);
      ...
      tft.setFreeFont(FSS12);
      tft.drawFloat( Isense2, 2, 175, 125, 4);
...
//      And now the negative voltage settings
  ...
  // This is the output voltage
      ...
      tft.drawFloat( abs(Vsense2), 2, 85, 65, 4);
...
/**********************************************************************************/
  ...
  // We need to monitor both pins, rising and falling for all states
    attachInterrupt(digitalPinToInterrupt(CLK2), rotary, HIGH);
    attachInterrupt(digitalPinToInterrupt(DATA2), rotary, HIGH);

  // ===============================================================================
  // Settings for ADC  
  //================================================================================  
 
    ads1115.begin(0x4A);  // Initialize ads1115 at the default address 0x48
    ads1115.setGain(GAIN_FOUR);  // 0.5mV steps, 1.024V max
...
// end of block */
  ...
  if (rotaryEncoder)
    {
      debugln("Rot_Enc 2 detected") ;   //if it works, use to call Check_Rot_Enc
    } 
  }
