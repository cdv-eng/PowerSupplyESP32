 ...
 *                  v14 29 June 2024 - Replaced delays with millis, and added optional INA219 libarary
...
//===============================================================================
  ...
  #define ENABLE_RLA 12             // O/P pin for output relay (optional)
...
//  gpio_num_t ADC_out_ref = (gpio_num_t) 25;
 ...
 // uint8_t dec_len = 2;                          // Number of decimals to display
  ...
  bool Precise = false, OldPrecise, SOAR, CL, OP, DT = false;    // Status
...
//  Iout = Isense0 ;
 ...
 Isense0 = ina219_A.getCurrent_mA();
  if (( 22 - Vsense0 ) * Iout ) { SOAR ; }

 // if (Iout || I2out > Ilimit)    // Need to remove this again
  if (Iout > Ilimit) 
  {
    CL = 1 ;
...
void Display_Output() 
  ...
  {     
      ...
      tft.drawFloat( abs( ( Vsense0 / 3.3 * 20 ) * 1.1429), 2, 175, 65, 4);

        // Current output
      tft.setTextColor(TFT_GREEN, TFT_BLACK);
      tft.setFreeFont(FSS12);
      tft.drawFloat (abs(Iout ), 2, 175, 100, 4);
 
      // This is the Current limiting
      tft.setTextColor(TFT_RED, TFT_BLACK);
      tft.setFreeFont(FSS12);
      tft.drawFloat( Ilimit, 2, 175, 130, 4); 
...
//      And now the negative voltage settings
      ...
      tft.drawFloat( abs((Vsense1 / 3.3 * 20) * 1.1834), 2, 85, 65, 4);

    // This is the current output
      tft.setTextColor(TFT_GREEN, TFT_BLACK);
      tft.setFreeFont(FSS12);
      tft.drawFloat( abs(I2out ) , 2, 85, 100, 4);
    

      // This is the current limit
      tft.setTextColor(TFT_RED, TFT_BLACK);
      tft.setFreeFont(FSS12);
      tft.drawFloat( Ilimit, 2, 85, 130, 4); 


      // This is to display the power (W) drawn

       tft.drawFloat(((Vsense0 / 3.3 * 20) * Iout ),2, 65, 200, 4);
    // tft.drawFloat( 2.3 ,2, 65, 200, 4 );
  }
void Check_button_press()
  {  
...
/*********************************************************************************/
  ...
  tft.drawRect(60, 50, 110, 120, TFT_YELLOW);

  // Channel headers
  tft.setTextColor(TFT_RED, TFT_BLACK);
  tft.setFreeFont(FSS9);
  tft.drawString("Channel 1", 35, 10, 4 );  // Channel label negative

  tft.setTextColor(TFT_YELLOW, TFT_BLACK);
  tft.setFreeFont(FSS9);
  tft.drawString("Channel 2", 165, 10, 4 );  // Channel label positive
  ...
  tft.drawString("V", 235, 65, 4);
  tft.setTextColor(TFT_GREEN, TFT_BLACK);
  tft.drawString("A", 150, 100, 4);
  tft.drawString("A", 235, 100, 4);
  tft.setTextColor(TFT_RED, TFT_BLACK);
  tft.drawString("A", 150, 130, 4);
  tft.drawString("A", 235, 130, 4);
